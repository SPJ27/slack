...
const CreateChannelModal = ({ onClose, router }: CreateChannelModalProps) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setStatus("idle");
      setErrorMessage(err instanceof Error ? err.message : "Failed to create channel");
    }
  };
    ...
    <div className="w-70 pb-10 shrink-0 bg-[#5E2C5f] text-white flex flex-col font-sans select-none relative overflow-x-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4">
        ...
        <div className="flex flex-col gap-[2px]">
          {channelsError && (
            <p className="px-2 py-1 text-xs text-red-300">{channelsError}</p>
          )}
          {channels.map((channel) => (
            <Channel
              ...
              active={String(channel.id) === params?.id}
              id={channel.id}
            />
          ))}
        </div>
